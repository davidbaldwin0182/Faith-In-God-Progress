...
class basicDetailViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        if let indexed = selectedName?.index {
            let myArticle = "myDocB\(indexed)\(names)"
    ...
    @IBAction func saveFileBtn(_ sender: Any) {
        ...
        let names = newUserUid
        if let indexed = selectedName?.index {
            let myDoc = "myDocumentBasNote\(indexed)\(names).txt"
    ...
    @objc func checkBtn(sender: UIBarButtonItem) {
        ...
        let names = newUserUid
        if let indexed = selectedName?.index {
            let myArticle = "myDocB\(indexed)\(names).txt"
            saveFile(document: myArticle)
            getCheckValue(document: myArticle)
            buttonVal = show(document: myArticle, buttonValue: buttonVal)
    ...
    func show() {
        ...
        let names = newUserUid
        if let indexed = selectedName?.index {
            let myDoc = "myDocumentBasNote\(indexed)\(names)"
