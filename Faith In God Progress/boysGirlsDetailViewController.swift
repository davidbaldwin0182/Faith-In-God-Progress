//
//  boysGirlsDetailViewController.swift
...
class boysGirlsDetailViewController: UIViewController, UIGestureRecognizerDelegate {
    
    var selectedName: DataItemB?
    ...
    override func viewDidLoad() {
        ...
        if let indexed = selectedName?.index {
            let myArticle = "myDocP\(indexed)\(names)"
    ...
    func updateMessage() {
        ...
        if buttonValBoys == "true" {
            message.text = selectedName?.message2
            studyTextView.text = selectedName?.study2
            if selectedName?.study2 == "" {
                studyTextView.isHidden = true
                studyTextView.isSelectable = false
                study.isHidden = true
                study.isEnabled = false
    ...
    @IBAction func saveFileBtn(_ sender: Any) {
        ...
        let names = newUserUid
        if let indexed = selectedName?.index {
            let myDoc = "myDocumentPreNote\(indexed)\(names).txt"
    ...
    @objc func checkBtn(sender: UIBarButtonItem) {
        ...
        let names = newUserUid
        if let indexed = selectedName?.index {
            let myArticle = "myDocP\(indexed)\(names).txt"
    ...
    func show() {
        ...
        let names = newUserUid
        if let indexed = selectedName?.index {
            let myDoc = "myDocumentPreNote\(indexed)\(names)"
